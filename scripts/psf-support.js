...
function isPsfEnabled() {
  return String(process.env.HAGIHUB_ENABLE_PSF || '').trim().toLowerCase() === 'true';
}
...
export function resolvePsfBuildConfig(projectRoot, arch) {
  ...
  const sourceDirectory = String(process.env.HAGIHUB_PSF_DIR || '').trim();

  if (!sourceDirectory) {
    throw new Error('HAGIHUB_PSF_DIR is required when HAGIHUB_ENABLE_PSF=true.');
  }
